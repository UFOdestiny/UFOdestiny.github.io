...
- `/publications/` builds its year jump nav by capturing `{% bibliography %}` and adding ids with `regex_replace` — nothing to maintain per year. The theme's `bibsearch.js` turns any URL hash into a filter term, so a small script in that page makes a year hash (`#2021`) clear the filter and scroll instead; without it, `#2021` listed every entry whose text mentions 2021. Per-entry anchors (`#yu2026trustenergy`) come from the gem; `about.md` links to them.
