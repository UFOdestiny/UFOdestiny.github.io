...
#
# jekyll-socials builds each icon's tooltip from the _data/socials.yml key with `key.gsub('_', ' ').capitalize` and offers no way to configure it, so the icons read "Github username", "Orcid id", "Scholar userid", "Cv pdf". Wrap the tag and rewrite the labels it produced. Anything not listed here keeps the gem's label.
#
# The tooltip is also the accessible name of an icon-only link, so each anchor gets an `aria-label` as well -- `title` alone is not reliably announced by a screen reader, and there is no text inside the <a> to fall back to.
